<template>
  <div class="guess-box">
    ...
    <div class="top-box">
      <div class="title"></div>
      <div class="balance-box" @click="showShop = true">
        <div class="balance">{{ conch }}</div>
        <!--        <div class="phone-box" >-->
        <!--          <button class="shop-btn"></button>-->
        <!--        </div>-->
      </div>
      <div class="msg-box">恭喜 用户昵称 在胡闹餐厅获得XXX*个</div>
      <button class="btn-close"></button>
      <button class="btn-help" @click="showRule = true"></button>
    </div>
    <!-- 近期记录 -->
    <div class="history-line-box" @click="showHistory = true">
    ...
    <div class="bottom-box">
      <div class="title">请选择本轮要订的食物</div>
      <div class="chose-box">
        <ul>
          <li v-for="item, index in giftArr" @click="choseThis(index)"
            :class="`${betedInfo[index + 1] > 0 ? 'win' : ''} ${choseGift === index ? 'active' : ''} `" :key="index">
            <div :class="`icon icon-${index + 1}`"></div>
      ...
      <div class="button-box" @click="movePx($event)">
        ...
        <div class="add-box">
          <button class="add-btn" @click="betThis(1)">
            <i></i><span>+1</span>
          </button>
          <button class="add-btn" @click="betThis(10)">
            <i></i><span>+10</span>
          </button>
          <button class="add-btn" @click="betThis(99)">
            <i></i><span>+99</span>
          </button>
        </div>
        <button class="last-again active" key="1" :disabled="!((currentSid - betSid) == 1 && !isOver)"
          @click="tryLastAgain">
        </button>
    ...
    <!--子页面-->
    <div class="enter-box" :class="`${showHistory || showRule || showShop ? 'show' : ''}`"
...
.top-box {
  ...
  .balance-box {
    ...
    height: 52px;
    min-width: 176px;
    background: url("~@/assets/kitchen/balance-bg.png") no-repeat center;
  ...
  .msg-box {
    ...
    top: 252px;
    left: 208px;
    background: url("~@/assets/kitchen/msg-bg.png") no-repeat center;
    background-size: contain;
    display: flex;
    align-items: center;
    justify-content: center;
    ...
    color: #FCF023;
    line-height: 28px;
    text-shadow: 0px 2px 0px #713905;
  }
  ...
  .btn-close {
    ...
    width: 48px;
    height: 48px;
    background: url("~@/assets/kitchen/icon-help.png") no-repeat center;
    background-size: contain;
    top: 252px;
    right: 16px;
  }
  ...
  .btn-close {
    ...
    width: 72px;
    height: 72px;
    background-image: url(~@/assets/kitchen/btn-close.png);
  }
...
.bottom-box {
  ...
  .title {
    ...
    color: #FFFFFF;
    line-height: 38px;
    text-shadow: 0px 4px 10px #500000;
    text-align: center;
    font-style: normal;
  }
    ...
    ul {
      ...
      li {
        ...
        .icon {
          ...
          width: 124px;
          height: 128px;
          background: url("~@/assets/kitchen/game-tab-1.png") no-repeat center -5px;
  ...
  .button-box {
    ...
    .add-box {
      ...
      .add-btn {
        ...
        height: 90px;
        position: relative;
        background: url("~@/assets/kitchen/btn-bet-bg.png") no-repeat center;
        background-size: contain;
        display: flex;
        flex-direction: column;
        align-items: center;
    ...
    .last-again {
      ...
      height: 100px;
      opacity: 1;
      background: url("~@/assets/kitchen/btn-repeat.png") no-repeat center;
      background-size: cover;
...
.enter-box {
  ...
  &.show {
    animation: enter 0.25s 1 linear forwards;
  }
}
